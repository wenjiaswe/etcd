...
func TestRollingDowngrade(t *testing.T) {
	...
	if err := ctlV3DowngradeEnable(cx, tv.String(),
		"The cluster is available to downgrade"); err != nil {
		cx.t.Fatalf("ctlV3DowngradeEnable error (%v)", err)
	}
...
func TestDowngradeThenDowngrade(t *testing.T) {
	...
	if err := ctlV3DowngradeEnable(cx, lowerTv.String(),
		"The cluster is available to downgrade"); err != nil {
		cx.t.Fatalf("ctlV3DowngradeEnable error (%v)", err)
	}
...
func ctlV3DowngradeEnable(cx ctlCtx, tv string, expected string) error {
	cmdArgs := append(cx.PrefixArgs(), "downgrade", "enable", tv)
	return spawnWithExpect(cmdArgs, expected)
}

func ctlV3DowngradeCancel(cx ctlCtx, expected string) error {
	cmdArgs := append(cx.PrefixArgs(), "downgrade", "cancel")
	return spawnWithExpect(cmdArgs, expected)
}

func ctlV3DowngradeValidate(cx ctlCtx, tv string, expected string) error {
	cmdArgs := append(cx.PrefixArgs(), "downgrade", "validate", tv)
	return spawnWithExpect(cmdArgs, expected)
}
